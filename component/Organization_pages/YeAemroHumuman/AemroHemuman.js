import React, { Component } from 'react'
import Routing from '../../Routing'
import AboveHeader from '../../AboveHeader'
import Footer from '../../Footer'
import DonateBY from '../../../pages/DonationBy'

class AemroHemuman extends Component {
    render() {
        return (
 <div>
            <AboveHeader/>
            <Routing/>
        <div>
        <div className="page-header">
            <div className="container">
                <div className="row">
                    <div className="col-12">
                        <h2>AemroHemuman Merja Organization</h2>
                         {/* <h2 style={{textAlign:'center'}}><DonateBY/></h2>  */}
                        {/* <h1><p><a href="/Donate">Donate now</a></p></h1> */}
                    </div>
                </div>
            </div>
        </div>
        
        <div className="single">
            <div className="container">
                <div className="row">
                    <div className="col-lg-8">
                        <div className="single-content">
                            <img src="img/amanuel.jpg" />
                            <h2>...</h2>
                            <p>
                               AemroHemuman Merja organizations is was established in 1998 
                               this is time gergeson help more than 500 mothers and other childers.
                                gergeson give them works for those who can works and help themself. 
                                this is time gergeson help more than 500 mothers and other childers 
                               </p>
                            <p>
                                this is time gergeson help more than 500 mothers and other childers.
                                gergeson give them works for those who can works and help themself. 
                                this is time gergeson help more than 500 mothers and other childers.
                                gergeson give them works for those who can works and help themself. 

                                this is time gergeson help more than 500 mothers and other childers.
                                gergeson give them works for those who can works and help themself. 
                                this is time gergeson help more than 500 mothers and other childers
                                this is time gergeson help more than 500 mothers and other childers.
                                gergeson give them works for those who can works and help themself. 
                                this is time gergeson help more than 500 mothers and other childers.
                                gergeson give them works for those who can works and help themself. 
                            
                            </p>
                        </div>
                        <div className="single-tags">
                            <a href="/Donate">kotebe aemhroHemuman
                            </a>
                        </div>
                        <div className="single-bio">
                            <div className="single-bio-img">
                                <img src="img/user.jpg" />
                            </div>
                            <div className="single-bio-text">
                                <h3>Head officer</h3>
                                <p>
                                    ato surafel is the head of gergeson charity organizations
                                    located in addis abeba in different kifle adisKetema.
                                </p>
                            </div>
                        </div> 
                    </div>

                    <div className="col-lg-4">
                        <div className="sidebar">
                            <div className="sidebar-widget">
                                <div className="search-widget">
                                    <form>
                                        <input className="form-control" type="text" placeholder="Search Keyword" />
                                        <button className="btn"><i className="fa fa-search"></i></button>
                                    </form>
                                </div>
                            </div>

                            <div className="sidebar-widget">
                                <h2 className="widget-title">Recent Post</h2>
                                <div className="recent-post">
                                    <div className="post-item">
                                        <div className="post-img">
                                            <img src="img/post-1.jpg" />
                                        </div>
                                        <div className="post-text">
                                            <a href="">this from gergeson aemhroHemuman
                                            </a>
                                            <div className="post-meta">
                                                <p><a href="/Donate">Donate now</a></p>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="post-item">
                                        <div className="post-img">
                                            <img src="img/post-2.jpg" />
                                        </div>
                                        <div className="post-text">
                                            <a href="">this from gergeson aemhroHemuman
                                            </a>
                                            <div className="post-meta">
                                                <p><a href="/Donate">Donate now</a></p>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="post-item">
                                        <div className="post-img">
                                            <img src="img/post-3.jpg" />
                                        </div>
                                        <div className="post-text">
                                            <a href="">this from gergeson aemhroHemuman
                                            </a>
                                            <div className="post-meta">
                                                <p><a href="/Donate">Donate now</a></p>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="post-item">
                                        <div className="post-img">
                                            <img src="img/post-4.jpg" />
                                        </div>
                                        <div className="post-text">
                                            <a href="">this from gergeson aemhroHemuman
                                            </a>
                                            <div className="post-meta">
                                                <p><a href="/Donate">Donate now</a></p>
                                                
                                            </div>
                                        </div>
                                    </div>
                                    <div className="post-item">
                                        <div className="post-img">
                                            <img src="img/post-5.jpg" />
                                        </div>
                                        <div className="post-text">
                                            <a href="">this from gergeson aemhroHemuman
                                            </a>
                                            <div className="post-meta">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>

                            <div className="sidebar-widget">
                                <div className="image-widget">
                                    <a href="#"><img src="img/blog-1.jpg" alt="Image" /></a>
                                </div>
                            </div>

                            <div className="sidebar-widget">
                                <div className="tab-post">
                                    <ul className="nav nav-pills nav-justified">
                                        <li className="nav-item">
                                            <a className="nav-link active" data-toggle="pill" href="#featured">Our Supporters</a>
                                        </li>
                                    
                                    </ul>

                                    <div className="tab-content">
                                        <div id="featured" className="container tab-pane active">
                                            <div className="post-item">
                                                <div className="post-img">
                                                    <img src="img/post-1.jpg" />
                                                </div>
                                                <div className="post-text">
                                                    <a href="">from bahardar</a>
                                                    <div className="post-meta">
    
                                                        <p><a href="/Donate">Donate now</a></p>
                                                    </div>
                                                </div>
                                            </div>
                                            <div className="post-item">
                                                <div className="post-img">
                                                    <img src="img/post-2.jpg" />
                                                </div>
                                                <div className="post-text">
                                                    <a href="">from adama</a>
                                                    <div className="post-meta">
    
                                                        <p><a href="/Donate">Donate now</a></p>
                                                    </div>
                                                </div>
                                            </div>
                                            <div className="post-item">
                                                <div className="post-img">
                                                    <img src="img/post-3.jpg" />
                                                </div>
                                                <div className="post-text">
                                                    <a href="">from mekele</a>
                                                    <div className="post-meta">
    
                                                        <p><a href="/Donate">Donate now</a></p>
                                                    </div>
                                                </div>
                                            </div>
                                            <div className="post-item">
                                                <div className="post-img">
                                                    <img src="img/post-4.jpg" />
                                                </div>
                                                <div className="post-text">
                                                    <a href="">from jima</a>
                                                    <div className="post-meta">
    
                                                        <p><a href="/Donate">Donate now</a></p>
                                                    </div>
                                                </div>
                                            </div>
                                            <div className="post-item">
                                                <div className="post-img">
                                                    <img src="img/post-5.jpg" />
                                                </div>
                                                <div className="post-text">
                                                    <a href="">from addisabeba</a>
                                                    <div className="post-meta">
                                                        <p><a href=""></a></p>
                                                        <p><a href="/Donate">Donate now</a></p>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div id="popular" className="container tab-pane fade">
                                            <div className="post-item">
                                                <div className="post-img">
                                                    <img src="img/post-1.jpg" />
                                                </div>
                                                <div className="post-text">
                                                    <a href="">from bahrdar</a>
                                                    <div className="post-meta">
                                                    </div>
                                                </div>
                                            </div>
                                            <div className="post-item">
                                                <div className="post-img">
                                                    <img src="img/post-2.jpg" />
                                                </div>
                                                <div className="post-text">
                                                    <a href="">from adama</a>
                                                    <div className="post-meta">
                                                    </div>
                                                </div>
                                            </div>
                                            <div className="post-item">
                                                <div className="post-img">
                                                    <img src="img/post-3.jpg" />
                                                </div>
                                                <div className="post-text">
                                                    <a href="">from diredewa</a>
                                                    <div className="post-meta">
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div id="latest" className="container tab-pane fade">
                                            <div className="post-item">
                                                <div className="post-img">
                                                    <img src="img/post-1.jpg" />
                                                </div>
                                                <div className="post-text">
                                                    <a href="">from addisabeba</a>
                                                    <div className="post-meta">
                                                    </div>
                                                </div>
                                            </div>
                                           
                                        </div>
                                    </div>
                                </div>
                            </div>

                            <div className="sidebar-widget">
                                <div className="image-widget">
                                    <a href="#"><img src="img/blog-2.jpg" alt="Image" /></a>
                                </div>
                            </div>
                            <div className="sidebar-widget">
                                <div className="image-widget">
                                    <a href="#"><img src="img/blog-3.jpg" alt="Image" /></a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
     <Footer/> 
  </div>
  </div>
        )
    }
}

export default AemroHemuman
;
